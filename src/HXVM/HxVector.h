 ...
 public:
  ...
  HxVector(uint32_t initSize) noexcept {
    ...
    while (this->data == NULL || this->data == nullptr) {
      usleep(retryDelay);
      fwprintf(this->errStream,
               HXVECTOR_LOG_ERR_LABEL L"分配失败，重试中...\n");
      this->data = new (std::nothrow) T[initSize];
    }
  ...
  ~HxVector() noexcept {
    ...
    if (this->logStream != nullptr)
      fwprintf(this->logStream, HXVECTOR_LOG_DEB_LABEL L"释放\n");
#endif
...
#ifdef HXVECTOR_DEBUG
    if (this->logStream != nullptr)
      fwprintf(this->logStream, HXVECTOR_LOG_DEB_LABEL L"所有元素已释放\n");
#endif
  }
