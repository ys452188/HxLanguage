...
typedef enum {
    OP_NOP,
    OP_LOAD_CONST, // 加载常量至栈顶 OP_LOAD_CONST <param_value> | OP_LOAD_CONST <const_index>
    OP_LOAD_VAR,   // 加载变量至栈顶
...
int readObjectCode(std::string path, ObjectCode& obj) {
    ...
    if (header[0] != 'H' || header[1] != 'X' || header[2] != 'O' || header[3] != 'C') {
        fclose(file);
        return -1;
    }
    // 2.读取常量池
    ...
    for (uint32_t i = 0; i < obj.constantPool.size; i++) {
        ...
        if (obj.constantPool.constants[i].type == CONST_STRING) {
            ...
            if (obj.constantPool.constants[i].value.string_value) {
                obj.constantPool.constants[i].size = (uint32_t)(wcslen(obj.constantPool.constants[i].value.string_value) * sizeof(wchar_t));
            }
        }
    }
    //3.读取过程
