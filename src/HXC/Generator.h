...
static void listObjectCode_Proc(Procedure *proc) {
    ...
    for (uint32_t i = 0; i < proc->instructionSize; i++) {
        ...
        case OP_DIV:
            ...
            break;
        case OP_CAL:
            fwprintf(logStream, L"\t%03u: \33[1;34mOP_CAL\33[0m %ls(funName), %u(paramCount)\n", i,
...
#endif
        objFun.push_back(generateFunction(program->functions[i], pitchTable, &(objCode->constantPool), program->functions, program->function_count, err));
...
Procedure *generateFunction(IR_Function *function, FunCallPitchTable& pitchTable, ConstantPool* constantPool,
                            IR_Function **all_functions,
                            int all_function_count, int *err) {
    if (!function || !err) {
        if (err)
    ...
    while (index < function->body_token_count) {
        ...
        if (wcscmp(currentToken.value, L"ret") == 0 ||
            ...
            ASTNode *expNode =
                parseExpression(function->bodyTokens, &index,
                                function->body_token_count, pitchTable, &localeSymbolTable, err);
            if (*err != 0 || !expNode) {
                delete (proc);
                return NULL;
            }
            ...
            generateInstructionsFromAST(proc->instructions, &inst_index,
                                        &inst_size, expNode, constantPool, err);
            if (*err != 0) {
                freeAST(expNode);
            ...
            index++;
            if((function->bodyTokens[index].type != TOK_VAL) || (function->bodyTokens[index].mark != STR)) continue;
            Instruction newInst = {};
            ...
            memcpy(newInst.params[0].value, &(strIndex),
                   sizeof(uint32_t));
...
void generateInstructionsFromAST(std::vector<Instruction>& instructions,
    ...
    if (node->kind == NODE_VALUE) {
        ...
        if (node->data.value.type.kind == IR_DT_INT) {
            ...
            constantPool->constants[constantPool->size].type = CONST_STRING;
            constantPool->constants[constantPool->size].value.string_value =
                (wchar_t *)calloc(wcslen(node->data.value.val.s) + 1,
                                  sizeof(wchar_t));
            if (!constantPool->constants[constantPool->size].value.string_value) {
                *err = -1;
                return;
        ...
        // 生成左子树指令
        generateInstructionsFromAST(
            instructions, inst_index, inst_size, node->left, constantPool, err);
        if (*err != 0) {
            return;
        }
        // 生成右子树指令
        generateInstructionsFromAST(
            instructions, inst_index, inst_size, node->right, constantPool, err);
        if (*err != 0) {
            return;
        }
        // 生成二元运算指令
        ...
        case 4: //SET
        {
            if(node->left->kind != NODE_VAR && node->left->kind != NODE_BINARY && node->left->data.binary.op != 4) {
                *err = 255;
        ...
        newInst.params[0].type = PARAM_TYPE_INDEX;
        memcpy(newInst.params[0].value, &(node->data.var.index),
               sizeof(uint32_t));
        newInst.params[0].size = sizeof(uint32_t);
        ...
        if (node->data.funCall.arg_count == 0) {
            ...
            for (uint32_t i = 0; i < node->data.funCall.arg_count; i++) {
                generateInstructionsFromAST(
                    instructions, inst_index, inst_size, node->data.funCall.args[i],
                    constantPool, err);
                if (*err != 0) {
                    return;
                }
        ...
        *********************************-*****--**********************/
        ...
        newInst.params[1].type = PARAM_TYPE_INT;
        memcpy(newInst.params[1].value,
               &(node->data.funCall.arg_count), sizeof(uint32_t));
