...
typedef struct Symbol {
    ...
    wchar_t* type;
    int type_arr_num;
    int* type_arr_size;  // 每层数组的大小,-1表示尚不知道
    bool isOnlyRead;
    bool isUsed;
    int sp;         // 栈中的位置
...
// 查找局部变量
int localeTable_search(wchar_t* name, Symbol* table, int size) {
    if (table == NULL || size == 0) return -1;
    for (int i = 0; i < size; i++) {
        if (wcscmp(name, table[i].name) == 0) return i;
    }
    return -1;
}
#include "Parser.h"
...
static int genVarDef(Tokens* tokens, int* index, Command** cmd, int* cmd_index,
                     int* cmd_size, Symbol* table, int table_size,
                     Symbol* sym) {
    ...
    if (wcscmp(tokens->tokens[*index].value, L"var") == 0 ||
        ...
        if (tokens->tokens[*index].type == TOK_END) {
            ...
            if (tokens->tokens[*index].type != TOK_END) {
                if (wcscmp(tokens->tokens[*index].value, L"=") == 0) {
                    if (*index + 1 >= tokens->count) {
                        setError(ERR_DEF_VAR, tokens->tokens[*index].line,
                                 tokens->tokens[*index].value);
                        return 255;
                    }
                    (*index)++;
        ...
        if (tokens->tokens[*index].type == TOK_END) {
            ...
            if (tokens->tokens[*index].type != TOK_END) {
                ...
                if (wcscmp(tokens->tokens[*index].value, L",") == 0 ||
                    ...
                    (*index)++;
                    if (wcscmp(tokens->tokens[*index].value, L"初始化") == 0) {
                        if (*index + 1 >= tokens->count) {
                            setError(ERR_DEF_VAR, tokens->tokens[*index].line,
                                     tokens->tokens[*index].value);
                            return 255;
...
// 生成load指令  (    load <size>     )
    ...
    if (sym->type != NULL) {
        if (wcscmp(sym->type, L"int") == 0 || wcscmp(sym->type, L"整型") == 0) {
            ...
            (*cmd)[*cmd_index].args[0].type = STRING;
    ...
    (*cmd_index)++;
    // 分析表达式
    if (sym->exp_begin != -1 && sym->exp_end != -1) {
#ifdef HX_DEBUG
        log(L"正在分析表达式");
#endif
        int begin = sym->exp_begin;
        ASTNode* ast = (ASTNode*)calloc(1, sizeof(ASTNode));
        if (!ast) return -1;
        int err = parseExp(tokens, begin, sym->exp_end, table, table_size, ast);
        if (err) return err;
        int genErr =genByAST(ast, cmd, cmd_size, cmd_index);
            ...
            if (wcscmp(fun->body->tokens[body_index].value, L"var") == 0 ||
                ...
                int err =
                    genVarDef(fun->body, &body_index, &(obj->body), &obj_body_index,
                              &(obj->body_size), localeTable, localeTable_size,
                              &(localeTable[localeTable_index]));
                if (err) return err;
                localeTable_index++;
