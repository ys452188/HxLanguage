...
wchar_t *keyword[] = { // 关键字
    L"var\0",      L"con\0",      L"fun\0",   L"class",      L"定义变量\0",
    L"定义常量\0", L"定义函数\0", L"定义类",  L"公有成员",   L"私有成员",
    L"受保护成员", L"public",     L"private", L"proctected", L"它的类型是",
    L"它的父类是", L"parent",     NULL};
  ...
  for (index_src = 0; index_src < length_src; index_src++) {
    ...
    if (src[index_src] == L'\'' || src[index_src] == L'‘' ||
      ...
      if (end_index == 0) {
        ...
        if (start_index + 1 < length_src)
          errCode[1] = src[start_index + 1];
      ...
      if (!(tokens->tokens[token_index].value))
        MEM_FAIL;
      if (len != 0)
  ...
  if (tokens->tokens) {
    ...
    for (int i = 0; i < (tokens)->count; i++) {
      ...
      fwprintf(logStream, L"(\"%ls\")",
                 tokens->tokens[i].value ? tokens->tokens[i].value
                                         : L"(NULL)\0");
