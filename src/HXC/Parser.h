...
typedef struct ASTNode {
  ...
  enum { NODE_VALUE, NODE_VAR, NODE_UNARY, NODE_BINARY, NODE_FUN_CALL } kind;
  union {
    struct {
      IR_DataType type;
      union {
    ...
    if (*index < size - 1) {
      ...
      if (next->type == TOK_OPR_LQUOTE) { // id({exp,})
        ...
        while (argIndex < size && tokens[argIndex].type != TOK_OPR_RQUOTE) {
          ...
          ASTNode *arg = parseExpression(tokens, &argIndex, size, table, err);
          if (*err != 0) {
            freeAST(funCallNode);
            return NULL;
          }
        ...
        *index = argIndex + 1;
        ...
        funCallNode->data.funCall.index = -1;
        // 获取函数索引和返回值类型
        if (table->fun_size > 0) {
          for (uint32_t i = 0; i < table->fun_size; i++) {
            ...
            if (wcscmp(f->name, funCallNode->data.funCall.name) == 0) {
              // 参数个数
              if (f->param_count != funCallNode->data.funCall.arg_count) {
                continue;
              }
              bool match = true;
              for (int j = 0; j < funCallNode->data.funCall.arg_count; j++) {
                ...
                if (funCallNode->data.funCall.args[j]->resultType.kind ==
                  ...
                  if (wcscmp(funCallNode->data.funCall.args[j]
                                 ->resultType.custom_type_name,
                             f->params[j].type.custom_type_name) != 0) {
                    match = false;
                    break;
                  }
                }
              ...
              if (match) {
                funCallNode->data.funCall.index = i;
                funCallNode->data.funCall.ret_type = f->return_type;
                funCallNode->resultType = f->return_type;
                return funCallNode;
              }
            }
          }
        ...
        // 即使当前作用域没有找到对应函数，也返回函数调用节点，后续阶段负责报错或链接
        return funCallNode;
      }
    } else {
      node->kind = NODE_VAR;
