    ...
    if (tokens->tokens[index].type == TOK_KW) {
      if (wcscmp(tokens->tokens[index].value, L"定义函数") == 0 ||
        ...
        program->functions = (IR_Function**)realloc(
            program->functions, program->function_count * sizeof(IR_Function*));
  ...
  if (tokens->tokens[*index].type == TOK_OPR_COMMA) {
    ...
    if ((*index + 1) >= tokens->count) {
      *err = 255;  // 语法错误
      setError(ERR_DEF_VAR, tokens->tokens[*index].line, NULL);
      if (var->type.kind == IR_DT_CUSTOM) {
        if (var->type.custom_type_name) free(var->type.custom_type_name);
      }
      free(var->name);
      free(var);
      return NULL;
  ...
  *param_count = 0;
  ...
  while (*index < tokens->count &&
    ...
    params = (IR_FunctionParam*)realloc(
        params, (*param_count) * sizeof(IR_FunctionParam));
    if (!params) {
      *err = -1;
      return NULL;
    }
