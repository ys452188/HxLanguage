        ...
        if (index == -255) {
          ...
          *err = 255;
          ...
          if (program->functions[index]->params) {
            for (int i = 0; i < program->functions[index]->param_count; i++) {
              if (program->functions[index]->params[i].name)
                free(program->functions[index]->params[i].name);
            }
            free(program->functions[index]->params);
          }
          if (program->functions[index]->return_type.custom_type_name)
            free(program->functions[index]->return_type.custom_type_name);
...
#endif
      function->return_type = parseDataType(tokens, index, tokens->count, err);
      if (*err) {
        // 释放已分配的内存
        for (int i = 0; i < function->param_count; i++) {
          free(function->params[i].name);
    ...
    if (member->type == IR_CM_VARIABLE) {
      if (member->data.variable->type.kind == IR_DT_CUSTOM) {
        ...
        IR_Class *custom_cls = getClassByName(
            member->data.variable->type.custom_type_name, currentIRProgram);
        if (custom_cls) {
