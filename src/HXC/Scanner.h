...
/*
  将 UTF-8 字节序列解码为 UTF-16（wchar_t），适配 Windows (wchar_t == 2) 和 Unix (wchar_t == 4)：
  - 输入: ptr 指向 UTF-8 数据，len 为字节长度
  - 输出: *out 为 null 结尾的 wchar_t* （由函数 malloc 出来，调用者负责 free），返回 0 成功，非 0 失败
*/
    ...
    *out = NULL;
    ...
    while (i < len) {
        ...
        if (c < 0x80) {
            ...
            // 110x yyyy
            if (i + 1 >= len) { free(wbuf); return -1; }
            unsigned char c1 = (unsigned char)ptr[i+1];
            if ((c1 >> 6) != 0x2) { free(wbuf); return -1; }
            codepoint = ((c & 0x1F) << 6) | (c1 & 0x3F);
            i += 2;
        } else if ((c >> 4) == 0xE) {
            if (i + 2 >= len) { free(wbuf); return -1; }
            unsigned char c1 = (unsigned char)ptr[i+1];
            unsigned char c2 = (unsigned char)ptr[i+2];
            if ((c1 >> 6) != 0x2 || (c2 >> 6) != 0x2) { free(wbuf); return -1; }
            codepoint = ((c & 0x0F) << 12) | ((c1 & 0x3F) << 6) | (c2 & 0x3F);
            i += 3;
        } else if ((c >> 3) == 0x1E) {
            if (i + 3 >= len) { free(wbuf); return -1; }
            unsigned char c1 = (unsigned char)ptr[i+1];
            unsigned char c2 = (unsigned char)ptr[i+2];
            unsigned char c3 = (unsigned char)ptr[i+3];
            if ((c1 >> 6) != 0x2 || (c2 >> 6) != 0x2 || (c3 >> 6) != 0x2) { free(wbuf); return -1; }
...
int readSourceFile(const char* path, wchar_t** src) {
    ...
    *src = NULL;
    ...
    if (size >= 3 &&
        ...
        size_t bytes = size - 2;
        if (bytes % 2 != 0) { free(buf); return -1; }
        size_t wcnt = bytes / 2;
        ...
        *src = out;
        ...
        for (size_t i = 0; i < wcnt; ++i) {
            ...
            // convert big-endian 16-bit to host endian (assume little-endian host on Windows)
